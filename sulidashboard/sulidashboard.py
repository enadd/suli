...
def total_sales(df_sl):
    ...
    september_data = df_sl[df_sl['Tanggal Order'].dt.month == 11]
...
def jumlah_barang_per_bulan(df_sl):
    ...
    grouped_long = grouped.reset_index().melt(id_vars='Nama Barang', 
                                               var_name='Bulan', 
                                               value_name='Quantity')
    
    # Create the bar chart using Plotly Express with color and text labels
    fig = px.bar(grouped_long, x='Nama Barang', y='Quantity',
...
def pie_jumlahbarang(df_sl):
    ...
    df_september = df_sl[df_sl['Tanggal Order'].dt.month == 9]
    
    # Group by 'JENIS' and count the occurrences
    quantity_counts = df_september.groupby('Nama Barang')['Quantity'].sum().reset_index(name='Count')
...
"""
    ...
    with col2:
        st.markdown(
        f"<h3>Total Customer: {total_customer(df_sl)}</h3>",
        unsafe_allow_html=True
        )
